...
dataset, H, gamma = 'cora'    , 1250, 10.**(-5)
#dataset, H, gamma = 'citeseer', 1500, 10.**(-6)
#dataset, H, gamma = 'pubmed'  , 1500, 10.**(-1)
#dataset, H, gamma = 'reddit'  , 1500, 10.**( 0)
num_executions = 10
...
A_hat_train = nontuple_preprocess_adj(A[train_idx, :][:, train_idx])
X_train     = X[train_idx, :]
N, C = X.shape
print(' Ok!')
...
for execution in range(num_executions):
    ...
    Y_test  = A_hat[test_idx,:] @ f(A_hat @ X @ W1) @ W2
